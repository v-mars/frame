...
import (
	...
	"github.com/v-mars/frame/pkg/jwt"
	"time"
)

var (
	JwtType string = "Bearer"
	JwtKey string = "xxx"
	HeaderTag string = "Authorization"
)

func TokenData(c *gin.Context) (map[string]interface{}, error) {
	hToken := c.Request.Header.Get(HeaderTag)
	var token string
	ticketLen := len(JwtType + " ") // Authorization AUTHORIZATION
	...
	var j = jwt.New()
	MapCla, err := j.ParseToken(token, JwtKey)
	if err != nil {
		return nil, err
	}
	return MapCla, nil
